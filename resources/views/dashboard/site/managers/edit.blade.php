                    ...
                    <div class="card">
                        <form action="{{ route('managers.update', ['manager' => $manager->id]) }}" method="post"
                            ...
                            <div class="card-body">
                                ...
                                @if($role->id == 2 && $role->name == 'doctor')
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-group">
                                            <label for="exampleInputPassword1">@lang('dashboard.doctor')</label>
                                            <select name="doctor_id"  class="form-control" required>
                                                <option selected disabled>@lang('dashboard.Choose_doctor')</option>
                                                @foreach($doctors as $doctor)
                                                    <option value="{{ $doctor->id }}" @if($doctor->id  == $manager->doctor->id) selected @endif>{{ $doctor->t('name') }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
